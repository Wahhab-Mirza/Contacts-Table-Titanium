...
//Rare exceptions to indicate a bug in the _contacts_table code (Eg, method that a subclass should have implemented)
...
+ (id)createProxy:(NSString*)qualifiedName withProperties:(NSDictionary*)properties inContext:(id<TiEvaluator>)context
{
	...
	if (proxyClass == nil) {
		NSString *_contacts_table = [NSString stringWithFormat:@"%@%s",@"Ti","tanium."];
		if ([qualifiedName hasPrefix:_contacts_table]) {
		...
		if (proxyClass==nil) {
			...
			@throw [NSException exceptionWithName:@"org.contacts_table.module"
